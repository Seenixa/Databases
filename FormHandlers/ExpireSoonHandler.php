...
<?php # Ügyfelenként és havi bontásban (év, hónap) kilistázni az ügyfél számláira érkező utalások összegét. #
    ...
    try {
        ...
        JOIN számlatípusok ON folyószámlák.típus_azonosító = számlatípusok.azonosító
        WHERE számlatípusok.meddig_érvényes > CURRENT_DATE
        GROUP BY felhasználók.név
        ORDER BY felhasználók.név;";
...
<body>
    ...
    if (empty($result)){
        ...
        echo "<br>";
        echo "A felhasználó(k), akiknek van olyan számlája amely hamarosan lejár.<br>";
        echo "*A megvalósításban az összes érvényes számla van listázva.<br>";
        echo "<br>";
    }
